...
- **BTFNT 分支预测器** — Backward-Taken Forward-Not-Taken 静态预测，正确预测时零停顿
- **分支配对发射** — 预测不跳转的分支可与后续指令同时发射，双发射率高达 50%
- **四级转发网络** — 周期内 / EX_MEM / MEM_WB / 寄存器文件，最大化指令吞吐
- **冒险处理** — RAW 转发、Load-Use 停顿、分支冲刷（仅误判时）、结构冒险检测
- **性能计数器** — 实时统计双发射率、分支误判率、停顿原因等流水线事件
- **MMU Sv32 分页** — 两级页表遍历 + 4MB 巨页支持 + TLB 缓存
- **中断异常** — ECALL/MRET/SRET、定时器/软件/外部中断、非法指令/地址未对齐检测
- **中断委托** — MIDELEG/MEDELEG 完整实现，支持将中断/异常委托至 S-mode；S-mode CSR 全量支持
- **外设** — UART 串口（跨平台键盘输入）、CLINT 定时器
- **裸机运行** — 10 个汇编测试 + 5 个性能基准测试 + 1 个贪吃蛇 C 游戏
...
| 结构冒险（双访存） | ID | 降级为单发射 |
| 控制冒险（分支：预测不跳→实际跳） | EX | 冲刷 IF_ID/ID_EX + PC 修正（under-prediction） |
| 控制冒险（分支：预测跳→实际不跳） | EX | 冲刷 IF_ID/ID_EX + PC 回退（over-prediction） |
| 控制冒险（分支：正确预测） | — | 零停顿（PC 已在 ID 阶段重定向） |
...
| Benchmark | IPC | 双发射率 | 说明 |
|-----------|-----|---------|------|
| Compute（纯 ALU） | 1.500 | 50.0% | 200k 迭代 ALU 循环 |
| BranchTest（分支密集） | 1.415 | 41.6% | 100k 次 if-else 决策树 |
| MemTest（访存密集） | 0.800 | 0.0% | 5k 次全数组遍历 |
| fib_test | 1.614 | — | Fibonacci 递归 |
| timer_test | 0.717 | — | 含定时器中断 |
| Snake（贪吃蛇） | ~0.65 | — | I/O + 分支 + 中断混合 |

## 局限与待改进

- BTFNT 为静态预测，动态分支准确率约 50%（前向分支）
- 固定 64KB 内存，无缓存层次
- S-Mode/U-Mode 特权级切换已支持，但未实现虚拟内存（页表仅在 M-mode 下工作）
- Load-Use 停顿在访存密集负载中占比仍高（~20%）
