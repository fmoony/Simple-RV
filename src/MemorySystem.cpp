...
// =========================================================================
// Cross-platform non-blocking keyboard check
// =========================================================================
...
// =========================================================================
// Double instruction fetch for dual-issue
// =========================================================================
...
bool MemorySystem::tlb_lookup(Word vpn, TranslationResult& result, bool is_write, bool is_fetch) const
{
    for (int i = 0; i < TLB_SIZE; ++i) {
        if (tlb[i].valid && tlb[i].vpn == vpn) {
            ...
            // Permission check
            if (is_fetch && !(flags & 0x4)) {  // X bit
                result.fault = true;
                result.cause = MCAUSE_INST_PAGE_FAULT;
                return false;
            }
            if (is_write && !(flags & 0x2)) {   // W bit
                result.fault = true;
                result.cause = MCAUSE_STORE_PAGE_FAULT;
                return false;
            }
            if (!is_write && !is_fetch && !(flags & 0x1)) { // R bit
                result.fault = true;
...
TranslationResult MemorySystem::translate(Addr vaddr, bool is_write, bool is_fetch, Word satp) const
{
    ...
    result.paddr = vaddr;  // Default: identity map
    result.fault = false;

    // Check SATP mode: bit 31 = 0 => Bare (no translation)
    Word mode = (satp >> 31) & 0x1;
    if (mode == 0) {
        return result;  // Bare mode: identity mapping
    }

    // Sv32 mode: 2-level page table walk
    Word vpn1 = (vaddr >> 22) & 0x3FF;  // VPN[1] = VA[31:22]
    Word vpn0 = (vaddr >> 12) & 0x3FF;  // VPN[0] = VA[21:12]
    Word offset = vaddr & 0xFFF;        // Page offset
    ...
    // Root page table: PPN from satp[21:0], shifted left by 12
    Addr root_ppn = satp & 0x003FFFFF;  // 22-bit PPN
    ...
    // Check if level-1 is a leaf (megapage: R or X set, D=1 for writable megapages)
    bool is_megapage = pte1_r || pte1_x;
    if (is_megapage) {
        // 4 MB megapage: PPN from level-1 PTE, offset from VA[21:0]
    ...
    // Level-0 PTE (pointer to next level)
