...
struct SystemConfig {
    ...
    // 工厂方法：生成默认的 128MB 开发板配置
    static SystemConfig Default128MB() {
        SystemConfig cfg;
        cfg.ram_size = MEMORY_SIZE;          // 128MB
        cfg.pc_init = 0x00000000;            // 入口指令在 0x0000
        cfg.sp_init = MEMORY_SIZE - 16;      // 栈顶设在物理内存最高处
        cfg.mtvec_init = 0x00010000;         // 默认中断入口（足够远避开程序主体）
        cfg.uart_base = 0x10000000;          // 串口硬件地址
        return cfg;
    }
    static SystemConfig Default64KB() { return Default128MB(); } // 兼容旧名称
};